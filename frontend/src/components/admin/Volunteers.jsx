...
const Volunteers = () => {
  ...
  const loadVolunteers = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://maaastha-website-etur.onrender.com/api/volunteers/all",
        {
  ...
  const handleVerify = async (id) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const res = await fetch(
        `https://maaastha-website-etur.onrender.com/api/volunteers/update/${id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const res = await fetch(
        `https://maaastha-website-etur.onrender.com/api/volunteers/delete/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const DetailRow = ({ icon: Icon, label, value, isFullWidth }) => (
    <div
      className={`flex items-start gap-3.5 ${isFullWidth ? "col-span-full" : ""}`}
    >
           {" "}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors duration-300 relative font-sans">
      ...
      <div className="p-7 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div>
                   {" "}
          <h2 className="text-2xl font-extrabold text-slate-950 dark:text-white font-heading">
            Volunteers Pool
          </h2>
                   {" "}
          <span className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 block">
            Review applications and manage NGO's support force
          </span>
                 {" "}
        </div>
               {" "}
        <div className="flex gap-2.5 bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-2xl w-fit border border-slate-200 dark:border-slate-700/50">
          ...
          <button
            ...
            className={`px-5 py-2.5 text-sm font-bold rounded-xl transition-all flex items-center gap-2 ${viewMode === "active" ? "bg-white dark:bg-slate-700 text-indigo-700 dark:text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-indigo-600"}`}
          >
                        New Applications{" "}
            {viewMode === "active" && (
              <span className="px-2 py-0.5 rounded-md bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 text-xs">
          ...
          <button
            ...
            className={`px-5 py-2.5 text-sm font-bold rounded-xl transition-all flex items-center gap-2 ${viewMode === "verified" ? "bg-white dark:bg-slate-700 text-emerald-700 dark:text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-emerald-600"}`}
          >
      ...
      <div className="min-h-[300px] px-2 pb-4 -mx-2">
        ...
        <div className="md:hidden space-y-4 px-2 mt-4">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
              No {viewMode === "active" ? "pending" : "verified"} applications
              found.
            </div>
          ) : (
            filteredVolunteers.map((v) => (
              <div
                key={v._id}
                className="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/50 p-5 rounded-2xl shadow-sm space-y-3"
              >
                               {" "}
                <div className="flex justify-between items-start">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-medium text-slate-500 mt-1">
                      {v.gender}, {v.age} yrs • {v.profession || "N/A"}
                    </div>
                                     {" "}
                  </div>
                                 {" "}
                ...
                               {" "}
                <div className="text-xs text-slate-500 border-b border-slate-100 dark:border-slate-700/50 pb-3">
                  Applied:{" "}
                ...
                <div className="flex flex-col gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-600 hover:text-white dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800/50 transition-all text-xs font-bold uppercase"
                  >
                  ...
                  {v.status !== "Verified" ? (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-600 hover:text-white dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-800/50 transition-all text-xs font-bold uppercase"
                    >
                                           {" "}
                  ...
                  {adminRole === "superadmin" && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-600 border border-red-100 dark:bg-red-900/40 dark:text-red-400 text-xs font-bold uppercase mt-1"
                    >
                                           {" "}
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="p-10 text-center text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700"
                  >
                    No {viewMode === "active" ? "pending" : "verified"}{" "}
                    applications found.
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="p-5 bg-white dark:bg-slate-800/90 border-y border-slate-200 dark:border-slate-700/50 first:border-l last:border-r first:rounded-l-2xl last:rounded-r-2xl shadow-sm group-hover:shadow-md group-hover:border-indigo-200 dark:group-hover:border-indigo-500/50 transition-all align-top">
                      ...
                      <div className="flex flex-col justify-center items-center gap-2">
                        ...
                        <button
                          onClick={() => setSelectedVol(v)}
                          className="w-full flex items-center justify-center gap-2 text-xs font-bold uppercase px-3 py-2 rounded-lg transition-all shadow-sm bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-600 hover:text-white dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800/50 dark:hover:bg-blue-600 dark:hover:text-white transition-colors"
                        >
                                                   {" "}
                        ...
                        {v.status !== "Verified" ? (
                          <button
                            onClick={() => handleVerify(v._id)}
                            className="w-full flex items-center justify-center gap-2 text-xs font-bold uppercase px-3 py-2 rounded-lg transition-all shadow-sm bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-600 hover:text-white dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-800/50 dark:hover:bg-emerald-600 dark:hover:text-white transition-colors"
                          >
                                                       {" "}
                            <CheckCircle size={15} strokeWidth={2.5} /> Verify  
                                                   {" "}
                        ...
                        {adminRole === "superadmin" && (
                          <button
                            onClick={() => handleDelete(v._id)}
                            className="w-full flex items-center justify-center gap-2 text-xs font-bold uppercase px-3 py-2 rounded-lg transition-all shadow-sm bg-rose-50 text-rose-600 border border-rose-100 hover:bg-rose-600 hover:text-white dark:bg-rose-900/40 dark:text-rose-400 dark:border-rose-800/50 dark:hover:bg-rose-600 dark:hover:text-white transition-colors opacity-0 group-hover:opacity-100"
                          >
                                                       {" "}
                            <Trash2 size={15} strokeWidth={2.5} /> Delete      
                                               {" "}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800 animate-fadeInUp">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2">
                                          <Contact size={14} /> ID:{" "}
                    <span className="font-mono bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded uppercase tracking-wider text-slate-700 dark:text-slate-300">
                      {selectedVol.idType} - {selectedVol.idNumber}
                    </span>
                                       {" "}
                  </p>
                                   
            ...
            <div className="p-8 overflow-y-auto space-y-9">
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-7 gap-x-8">
                ...
                <DetailRow
                  icon={Cake}
                  label="Age"
                  value={`${selectedVol.age} Years`}
                />
                                 
                <DetailRow
                  icon={User}
                  label="Gender"
                  value={selectedVol.gender}
                />
                                 
                <DetailRow
                  icon={Briefcase}
                  label="Profession"
                  value={selectedVol.profession}
                />
                             {" "}
              </div>
                           {" "}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-1 space-y-7 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
                                     {" "}
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white border-l-4 border-indigo-500 pl-3">
                    Contact Info
                  </h4>
                                     {" "}
                  <DetailRow
                ...
                <div className="md:col-span-2 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 h-full">
                                     {" "}
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white border-l-4 border-indigo-500 pl-3 mb-7">
                    Permanent Address
                  </h4>
                                     {" "}
                  <DetailRow
              ...
              <div className="bg-indigo-50/50 dark:bg-indigo-950/20 p-7 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-1.5 flex items-start gap-3.5">
                                           
                    <div className="mt-0.5 p-2.5 rounded-xl bg-white dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 border dark:border-indigo-800">
                      <HeartHandshake size={19} />
                    </div>
                                           
                    <div>
                                               {" "}
                      <p className="text-xs font-bold text-indigo-900/60 dark:text-indigo-400 uppercase tracking-wide">
                        Areas of Help
                      </p>
                                               {" "}
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-1 leading-relaxed">
                        {selectedVol.helpText}
                      </p>
                                             
                    </div>
                                       {" "}
                  ...
                  <div className="space-y-1.5 flex items-start gap-3.5">
                    ...
                    <div>
                                               {" "}
                      <p className="text-xs font-bold text-indigo-900/60 dark:text-indigo-400 uppercase tracking-wider">
                        Availability Slot
                      </p>
                                               {" "}
                      <p className="text-base font-semibold text-indigo-950 dark:text-white mt-1 leading-relaxed bg-white dark:bg-indigo-950 px-3 py-1.5 rounded-lg border dark:border-indigo-800 w-fit">
                        {selectedVol.availability}
                      </p>
                                             
                    </div>
                                       {" "}
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center gap-3.5">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-500">
                System Status:{" "}
                <strong
                  className={`ml-1 px-2 py-0.5 rounded ${selectedVol.status === "Verified" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}
                >
                  {selectedVol.status}
                </strong>
              </span>
